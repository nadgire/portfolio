import React, { useEffect, useState, useRed } from 'react'
            ...
            {/* <div className='flex flex-wrap max-w-[80%] mx-auto pt-14 justify-around gap-y-5'>
                {
                    educationData.map((x, index) => {
                        return (
                            <motion.div onscreen={{ bounce: 0.4 }} transition={{ duration: 1, bounce: 1 }} initial={{ opacity: 0, translateY: 80 }} whileInView={{ opacity: 1, translateY: 0 }} key={x.id} className="flex flex-col md:flex-row gap-2 md:gap-0 bg-white shadow-md rounded-lg overflow-hidden w-full max-w-3xl border">
                                <img
                                    src={x.image}
                                    alt={x.college}
                                ...
                                <div className="ml-4 flex flex-col justify-center py-2 md:py-0">
                                    ...
                                    <p className="text-green-600 font-semibold mt-1">{x.duration} | {x.status}</p>
            ...
            <div className='flex flex-wrap max-w-[80%] mx-auto pt-14 justify-around gap-y-5'>
                {
                    educationData.map((x, index) => {
                        return (
                            <motion.div
                                key={x.id}
                                initial={{ opacity: 0, y: 80 }} // Offscreen start
                                whileInView={{
                                    opacity: 1, // Fade in
                                    y: [80, -20, 15, -10, 5, 0], // Bounce effect
                                }}
